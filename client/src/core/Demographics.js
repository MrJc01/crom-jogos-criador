...
export class Demographics {
    ...
    processDTM(eraMult, hasSanitation, biomeId, deltaDays = 1, globalPop = 0) {
        ...
        // FIX BALANCE: Populações fundadoras não podem ser extintas por variância estocástica
        if (this.total < 1000) {
            chaosFactorDeaths = Math.min(chaosFactorDeaths, 1.5);
        }
        ...
        // CAP CÓSMICO DEFENSIVO: Teto absoluto de 10 milhões por hexágono
        // Impede estouros de float/BigInt em ticks massivos de background (ex: jogo rodando a noite toda)
        const COSMIC_CAP = 10_000_000;
        if (this.total > COSMIC_CAP) {
            this.total = COSMIC_CAP;
        }
        
        // Freio logarítmico: quando a pop se aproxima de 80% do cap, nascimentos desaceleram
        // Isso impede saltos bruscos de "abaixo do cap" para "muito acima"
        if (this.total > COSMIC_CAP * 0.8) {
            const overshootRatio = this.total / COSMIC_CAP;
            const brakeFactor = Math.max(0.01, 1.0 - overshootRatio);
            // Reduz a fila de gestação proporcionalmente
            for (let i = 0; i < this.pregnancyQueue.length; i++) {
                this.pregnancyQueue[i] = Math.floor(this.pregnancyQueue[i] * brakeFactor);
            }
        }
