...
async function run() {
    ...
    // Inject logging into plugins
    engine.plugins.forEach(plugin => {
        const oldApply = plugin.applyTick;
        plugin.applyTick = function(node, globalRules, eng) {
            let t0 = process.hrtime.bigint();
            oldApply(node, globalRules, eng);
            let t1 = process.hrtime.bigint();
            if(!times[plugin.id]) times[plugin.id] = 0n;
            times[plugin.id] += (t1 - t0);
        }
    });
    ...
    for (const [id, time] of Object.entries(times)) {
        console.log(`${id}: ${Number(time) / 1000000} ms`);
    }
    console.log("Done");
}
run();
